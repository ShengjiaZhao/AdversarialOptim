...
class CifarDataset(Dataset):
    def __init__(self, one_hot=False, use_edge=False):
        ...
        for i in range(5):
            content = self.unpickle(os.path.join(os.path.dirname(os.path.realpath(__file__)), self.folder, 'data_batch_%d' % (i+1)))
        ...
        if use_edge:
            edge_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), self.folder, "canny_edge")
        ...
        content = self.unpickle(os.path.join(os.path.dirname(os.path.realpath(__file__)), self.folder, 'test_batch'))
        data, labels = content[b'data'], content[b'labels']
        ...
        # Load label names
        self.label_names = self.unpickle(os.path.join(os.path.dirname(os.path.realpath(__file__)), self.folder, 'batches.meta'))[b'label_names']
